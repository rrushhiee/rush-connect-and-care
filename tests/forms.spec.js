...
test("enquiry form submits successfully", async ({ page }) => {
  await page.goto("http://127.0.0.1:4173/enquire");

  await page.route("https://api.web3forms.com/submit", async (route) => {
    await route.fulfill({
      status: 303,
      headers: {
        location: "http://127.0.0.1:4173/thank-you"
      }
    });
  });
...
test("enquiry form is wired to Web3Forms", async ({ page }) => {
  ...
  await expect(form.locator('input[name="redirect"]')).toHaveValue("https://rushconnectandcare.com.au/thank-you");
});

test("booking form submits successfully", async ({ page }) => {
  await page.goto("http://127.0.0.1:4173/book-appointment");
