...
function looksLikeEmail(value) {
  return EMAIL_PATTERN.test(normaliseValue(value));
}

function looksLikePhone(value) {
  const digits = normaliseValue(value).replace(/\D/g, "");
  return digits.length >= 8;
}

function getOrCreateSessionId() {
  const existing = safeSessionStorage("getItem", SESSION_ID_STORAGE_KEY);

  if (existing) {
    return existing;
  }

  const created = `rcc-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  safeSessionStorage("setItem", SESSION_ID_STORAGE_KEY, created);
  return created;
}
...
function rememberAttribution() {
  ...
  const current = {
    utm_source: normaliseValue(params.get("utm_source")),
    utm_medium: normaliseValue(params.get("utm_medium")),
    utm_campaign: normaliseValue(params.get("utm_campaign"))
  };
  ...
  safeSessionStorage("setItem", ATTRIBUTION_STORAGE_KEY, JSON.stringify(merged));
  getOrCreateSessionId();
  return merged;
}

function buildLeadFieldMap(form) {
  const values = {};
  ...
  for (const [key, value] of formData.entries()) {
    if (!(key in values)) {
      values[key] = normaliseValue(value);
    }
  }
...
function inferPlanStatus(values) {
  const combined = [
    values.plan_status,
    values.plan_managed,
    values.ndis_plan,
  ...
  return /\b(ndis|plan-managed|plan managed|self-managed|self managed|plan manager)\b/.test(combined)
    ? "yes"
    : "no";
}

function inferEmail(values) {
  const candidates = [values.email, values.contact_details];
  return candidates.find((value) => looksLikeEmail(value)) || "";
}

function inferPhone(values) {
  const candidates = [values.phone, values.contact_details];
  return candidates.find((value) => looksLikePhone(value)) || "";
}

function inferBookingWithin7Days(values) {
  const dateKeys = ["preferred_date", "date", "appointment_date", "booking_date", "session_date"];
  const now = Date.now();
  const sevenDays = 7 * 24 * 60 * 60 * 1000;

  for (const key of dateKeys) {
    const raw = normaliseValue(values[key]);
    if (!raw) {
      continue;
    }
    ...
    if (parsed >= now && parsed - now <= sevenDays) {
      return true;
    }
  }
...
function storeLeadSuccessPayload(form) {
  const values = buildLeadFieldMap(form);
  ...
  const payload = {
    ...
    booking_within_7days: inferBookingWithin7Days(values),
  ...
  safeSessionStorage("setItem", LEAD_STORAGE_KEY, JSON.stringify(payload));
}
